...
```{r ch3, message=FALSE, cache = F, eval = F}
...
```{r ch5, message=FALSE, cache = T, echo = T, eval = F}
...
```{r ch6, message=FALSE, cache = T, echo = T}

datos <- read.csv("https://github.com/jabiologo/web/raw/master/nmixDatos.csv")
head(datos)

cober <- rast("https://github.com/jabiologo/web/raw/master/cober.tif")
tempe <- rast("https://github.com/jabiologo/web/raw/master/tempe.tif")
names(cober) <- "cober"
names(tempe) <- "tempe"
par(mfrow = c(1,2))
plot(tempe, main = "Temperatura")
points(xyFromCell(elev, datos$id), pch = 19)plot(arbu, main = "Porcentaje arbusto")
plot(cober, main = "Cobertura del suelo")
datos$tempe <- scale(tempe[datos$id])[1:100]
datos$cober <- scale(cober[datos$id])[1:100]
head(datos)
```
