...
\begin{equation}
\tag{Eq 3}\label{eq3}
\lambda_i = exp(3 - (0.9 * dwat) + (0.15 * forest))
\end{equation}
<br/><br/> 
...
Con estos 50 datos, podríamos intentar ajustar un modelo. Es decir, podríamos relacionar la abundancia de pinzones (nuestra variable respuesta), con los datos que hemos tomado de distancia al punto de agua y de cobertura vegetal (nuestros predictores). A partir de los resultados, podríamos intetar hacer inferencias sobre cómo afectan estos dos factores a la abundancia de pinzones. Vamos a ello...
...
En el resumen de nuestro modelo, los "Estimates" son las $\beta$ de nuestro modelo, siendo el intercepto la $\beta_0$, y las otras dos las $\beta$ correspondientes a los predictores "distancia al punto de agua" y "cobertura forestal". Como en realidad fuimos nosotros quienes simulamos el proceso, tenemos acceso a los _estimates_ reales con los que la abundancia de pinzones fue creada. Ahora, ¡podemos comparar los _estimates_ reales con los que hemos obtenido a partir de nuestro modelo!
